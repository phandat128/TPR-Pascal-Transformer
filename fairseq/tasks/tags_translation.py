...
from fairseq.data import (
    ...
    data_utils,
    encoders,
    indexed_dataset, LanguagePairDataset,
)
...
@dataclass
class TagsTranslationConfig(TranslationConfig):
    tags_data: Optional[str] = field(
        default=None,
        metadata={
...
# @dataclass
# class TagsTranslationConfig(FairseqDataclass):
...
@register_task("tags_translation", dataclass=TagsTranslationConfig)
class TagsTranslationTask(FairseqTask):
    ...
    cfg: TagsTranslationConfig

    def __init__(self, cfg: TagsTranslationConfig, src_dict, tgt_dict, src_tags_dict):
        super().__init__(cfg)
    ...
    @classmethod
    def setup_task(cls, cfg: TagsTranslationConfig, **kwargs):
        ...
        if cfg.tags_data is None:
            raise Exception('Path(s) to tags data directorie(s) missing')
        src_tags_dict = cls.load_dictionary(os.path.join(cfg.tags_data, 'dict.{}.txt'.format(cfg.source_lang)))
        print('| [{}] tags dictionary: {} types'.format(cfg.source_lang, len(src_tags_dict)))

        return cls(cfg, src_dict, tgt_dict, src_tags_dict)
