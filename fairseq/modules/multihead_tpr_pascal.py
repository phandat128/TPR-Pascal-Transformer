...
class MultiheadTPRPascal(FairseqIncrementalDecoder):
    ...
    def forward(
        ...
        rng = torch.FloatTensor(range(parents.size(1))).to(attn_weights.device)
        pascal_weights = self.weight_fn(rng, parents, self.weight_param)\
            .repeat(1, self.num_pascal_heads, 1)\
            .view(-1, parents.size(1), parents.size(1))

        if self.training and self.parent_ignoring > 0.:
            samples = self.binomial.sample(pascal_weights.size()[:2]).type(attn_weights.dtype).to(attn_weights.device)
